...
import ru.purpir.client.MinecartChainClientState;
...
public final class MinecartChainRenderer {
    ...
    private static final float LINK_SIZE = 0.25F;

    private MinecartChainRenderer() {
    }

    public static void register() {
        WorldRenderEvents.AFTER_ENTITIES.register(MinecartChainRenderer::render);
    }

    private static void render(WorldRenderContext context) {
        MinecraftClient client = MinecraftClient.getInstance();
        if (client.world == null) {
            MinecartChainClientState.clear();
            return;
        }
        ...
        for (MinecartChainClientState.Link link : MinecartChainClientState.getLinks()) {
            ...
            float vMax = (float) (length / LINK_SIZE);
